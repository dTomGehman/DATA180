...
library(tidyverse)
library(tm)
setwd("C:/Users/gehmand/OneDrive - Carlisle Area School District/Desktop/Data 180 dTomGehman/DATA180/hw7")
...
```{r}
levels(factor(news$year)) #19 levels, so 19 years
```
...
wordCorpus[[1]]$content #first headline
```
...
```{r}
#a term document matrix is a matrix containing frequencies of words (terms) in the collection of documents
...
```{r}
posmatch = wordCounts[match(names(wordCounts), posWords, nomatch = 0) != 0]
barplot(posmatch[posmatch>=20], las=2, cex.names=.75)
sum(posmatch)/sum(wordCounts) #3.7% positive words
negmatch = wordCounts[match(names(wordCounts), negWords, nomatch = 0) != 0]
barplot(negmatch[negmatch>=20], las=2, cex.names=.75)
sum(negmatch)/sum(wordCounts) #7.8% negative words
```
...
```{r}
factor(news$yearmonth, order=T)
ggplot(news, aes(factor(yearmonth, order=T))) + geom_bar() + theme(axis.text=element_text(size=4,angle=90))
#The number of articles released per month slowly increased through 2013 and then started to decrease.  
```
...
```{r}
library("quanteda")
library('corpus')#I installed this based off of an older version.  
```
...
```{r}
newts = term_stats(wordVector, text_filter(drop_symbol=T, drop=stopwords_en, drop_punct=T, drop_number=T))
newts = arrange(newts, desc(count))
head(newts, 20)
```
...
```{r}
newts = term_stats(wordVector, text_filter(drop_symbol=T, drop=stopwords_en, drop_punct=T, drop_number=T), ngrams=2)
newts = arrange(newts, desc(count))
head(newts, 20)
```
...
```{r}
newscorpus = corpus_reshape(corpus(news$headline_text), to="paragraphs")
head(newscorpus)
```
...
```{r}
#toks=tokens(newscorpus, remove_punct=T, remove_symbols=T, remove_numbers=T)
...
#I was not surprised to see many of the words.  
#Words like "police" and "say" are common in headlines
#given that these are Australian, words involving Australia and its towns and territories weren't surprising.  
#I was a bit surprised that "woman" is much less common than "man"; I'd expect them to be a bit more similar.  
#I was also surprised that there aren't as many words about finances or economics; there's much more about crime/disaster and some political terms.  
```
...
```{r}
betas <- tidy(topic_model, matrix = "beta")
betas
...
top_topics_b %>%
  ...
  scale_y_reordered()
# Topics 4 and 6 seem to be about similar news, but 4 involves the northern part, and 6 involves the southern part.  They both involve "police," "charge," and "win."  
# Topic 3 clearly involves health.  It contains "health," "hospital," and "death" in the first 10.  Looking down the list past top 10, "test" and "coronavirus" are also there.  
# Topic 1 involves politics.  It has words like "plan" and "council," which suggest gov't work, and it has "us" and "warn," which may suggest global relations as well. I looked in the top 20, and "elect," "market," "case," and "job" are there as well.  
```
...
```{r}
gammas <- tidy(topic_model, matrix = "gamma")
#gammas
...
top_g_one = mutate(filter(top_g, topic==1), id=as.numeric(substring(document, 5)))#obtain headlines for first topic.  id is the column with numbers representing the documents
top_g_one = mutate(top_g_one, contents = newscorpus[id]) #index id to get actual text content
top_g_one
#these headlines aren't all closely related, but they seem to deal with jobs and some other news.  
#I reran the code with n=20 in line 242, and the headlines mostly regard either jobs and international news.  It seems like these and other minor things were lumped into one topic.  
top_g_two = mutate(filter(top_g, topic==2), id=as.numeric(substring(document, 5)))
top_g_two = mutate(top_g_two, contents = newscorpus[id])
top_g_two
#this topic is clearly about murders and other crimes, and the headlines reflect that.  
```
