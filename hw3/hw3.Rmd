...
```{r}
ggplot(Cars93, aes(EngineSize, MPG.city)) + geom_point(col="blue", size=2) + labs(title="Car Engine Sizes and Gas Mileage", subtitle = "Cars93 Dataset", x="Engine Size (Liters)", y="City Gas Mileage (MPG)", caption="Using GGPlot geom_point to plot 2 variables, EngineSize and MPG.city.")+xlim(1,5.7)+ylim(15,47)
...
```{r}
ggplot(Cars93, aes(EngineSize, MPG.city, colour=Type)) + geom_point(size=2) + labs(title="Car Engine Sizes and Gas Mileage", subtitle = "Cars93 Dataset", x="Engine Size (Liters)", y="City Gas Mileage (MPG)", caption="Using GGPlot geom_point to plot 2 variables, EngineSize and MPG.city.")+xlim(1,5.7)+ylim(15,47)
```
...
```{r}
ggplot(Cars93, aes(EngineSize, MPG.city, colour=Type)) + geom_point(size=2) + labs(title="Car Engine Sizes and Gas Mileage", subtitle = "Cars93 Dataset", x="Engine Size (Liters)", y="City Gas Mileage (MPG)", caption="Using GGPlot geom_point to plot 2 variables, EngineSize and MPG.city.")+xlim(1,5.7)+ylim(15,47)+geom_smooth(linetype="dashed", color="red")
```
...
```{r}
ggplot(Cars93, aes(AirBags)) + geom_bar(fill=c("red", "green", "blue")) + labs(title="Airbags in Cars", subtitle="Cars93 Dataset", caption="Using GGPlot goem_bar to plot frequencies of numbers of airbags equipped in cars.", x="Airbags Equipped", y="Frequency")
```
...
```{r}
ggplot(Cars93, aes(Origin, fill=AirBags)) + geom_bar(position='dodge') + labs(title="Airbags in Cars, Foreign & Domestic", subtitle="Cars93 Dataset", caption="Using GGPlot goem_bar to plot frequencies of six categories:  \nUS & non-US Cars, each split by 3 airbag equipment levels.", x="Airbags Equipped", y="Frequency")
```
...
```{r}
ggplot(au_states)+geom_sf(aes(fill=NAME))
```
## Task 3
...
```{r}
ggplot(au_states)+geom_sf(aes(fill=NAME))+geom_point(data=au_capitals, aes(lon, lat), size=2, col='red')
```
...
```{r}
ggplot(au_states)+geom_sf(aes(fill=NAME))+geom_point(data=au_capitals, aes(lon, lat), size=2, col='red')+coord_sf(crs=st_crs(3112))
...
```{r}
mapview(au_states)+mapview(capitals_geo, legend=FALSE, col.regions='red')
```
